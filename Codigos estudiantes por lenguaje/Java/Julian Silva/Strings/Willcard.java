...
// Llena la tabla
		for (int i = 1; i <= x; i++) {
			for (int j = 1; j <= y; j++) {
				/*
				 * Dos casos que podemos ver de '*' a) Ignoramos '*' y pasamos al siguiente
				 * caracter en el patrón b) '*' coincide con un iesimo caracter en una entrada
				 */

				if (pattern.charAt(j - 1) == '*') {
					verificacion[i][j] = verificacion[i][j - 1] || verificacion[i - 1][j];
				} /*
					 * Actuales caracteres son considerados como coincidentes en dos casos a) actual
					 * caracter de patron es '?' b) caracteres actuales coinciden
